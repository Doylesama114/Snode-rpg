...
### 修复
- **费用/战力颠倒**：103 张卡牌 `cost` 与 `basePower` 与 xlsx 权威源对齐（含双足飞龙 3 费 7 战、雷龙 6 费 12 战等）
- **雪怪**：冰环境减费后 `playCostValue` 修正为 1
- 新增 `scripts/audit-cost-power.mjs` / `fix-cost-power-from-xlsx.mjs` 回归校验

## [1.0.7008] - 2026-07-08

### 修复
- **篝火**：roundEnd 战力加成改为持久化，不再被 `recalculateAllPowers` 抹掉
- **兽栏 / 海港 / 温馨的旅馆**：额外槽创建与部署；点击宿主本体可自动解析到额外槽
- **寒脊山脉 / 温馨的旅馆 / 回春术 / 箭袋**：同类回合战力变化统一走持久加成
- **首回合自动进场**：触发 `createSlot` 时正确创建额外槽
