...
class EffectManager {
  ...
  static buildRevealBatch(game) {
    ...
    entries.sort((a, b) => a.playerIndex - b.playerIndex || a.orderIndex - b.orderIndex)
    return entries
  }
  ...
  static resolveRevealBatch(game) {
    ...
    for (const entry of batch) {
      ...
      if (player.currentCost < 0) {
        EffectManager.voidPendingRevealEntry(entry, game, messages)
        continue
      }
    ...
    messages.push(...EffectManager.applyBatchHighestFreeDeploy(
  ...
  static isEquipAttachmentCard(card) {
    if (!EffectManager.getDeployOnHostEffect(card) || EffectManager.isVehicleMountCard(card)) return false
    if (EffectManager.hasAnyKeyword(card, ['护甲', '武器'])) return true
    const effect = EffectManager.getDeployOnHostEffect(card)
    return EffectManager.hasAnyKeyword(card, ['物件']) && effect.requireHostCardType === 'unit'
  }

  static usesAttachmentSlot(card) {
    return EffectManager.isEquipAttachmentCard(card) || EffectManager.isVehicleMountCard(card)
  }

  static getEquipSlotKind(card) {
    if (EffectManager.hasAnyKeyword(card, ['护甲'])) return 'equipArmor'
    if (EffectManager.hasAnyKeyword(card, ['武器', '物件'])) return 'equipWeapon'
    return null
  }

  static getHostFieldIndex(player, hostCard) {
    return player.field.findIndex(s => s.card === hostCard)
  }

  static findExtraSlotOnHost(player, hostIdx, predicate) {
  ...
  static resolveAttachmentSlotIndex(player, hostIdx, card, createIfMissing = true) {
    if (hostIdx < 0) return -1

    if (EffectManager.isEquipAttachmentCard(card)) {
      const kind = EffectManager.getEquipSlotKind(card)
      if (!kind) return -1
      if (EffectManager.findExtraSlotOnHost(player, hostIdx, s => s.slotKind === kind && s.card) >= 0) {
        return -1
      }
    ...
    if (EffectManager.isVehicleMountCard(card)) {
      const emptyVehicle = EffectManager.findExtraSlotOnHost(player, hostIdx, s =>
        !s.card && (s.slotKind === 'vehicle' || !s.slotKind) && EffectManager.canDeployOnExtraSlot(card, s),
      )
      if (emptyVehicle >= 0) return emptyVehicle
      if (EffectManager.findExtraSlotOnHost(player, hostIdx, s =>
  ...
  static applyHostDeployBonusesToCard(card, hostCard) {
    const messages = []
    const effect = EffectManager.getDeployOnHostEffect(card)
    if (effect?.hostBonusIfHostAttribute && hostCard.attribute === effect.hostBonusIfHostAttribute) {
  ...
  static deployAttachmentOntoHost(card, hostCard, player, game) {
    const gameState = game.gameState || game
    const messages = []
    const hostIdx = EffectManager.getHostFieldIndex(player, hostCard)
    const slotIdx = EffectManager.resolveAttachmentSlotIndex(player, hostIdx, card, true)
    if (slotIdx < 0) {
    ...
    card.excludeFromFieldCount = true
    messages.push(...EffectManager.applyHostDeployBonusesToCard(card, hostCard))
    messages.push(...EffectManager.applyExtraSlotDeployModifiers(card, slot))
    if (!card.excludeFromFieldCount) card.excludeFromFieldCount = true
    slot.card = card
    messages.push(`${card.name} 部署到 ${hostCard.name}`)

    card.effects?.forEach(effect => {
      if (effect.timing === 'onDeploy' || (card.quickPlay && effect.timing === 'onPlay')) {
        const result = EffectManager.applyDeployEffect(effect, card, player, gameState)
  ...
  static applyDeployOntoHost(card, hostCard, player, game) {
    if (EffectManager.usesAttachmentSlot(card)) {
      return EffectManager.deployAttachmentOntoHost(card, hostCard, player, game)
    }
    const gameState = game.gameState || game
    const messages = []
    card.deployOnCardTarget = hostCard.id
  ...
  static getQuickPlayHostTargets(player, card) {
    ...
    return player.field
      .filter(s => s.card && !s.isExtra && EffectManager.canAttachToHost(card, s.card, player))
      .map(s => s.card)
  }
...
class GameEngine {
  ...
  handlePlayCard(playerId, cardIndex, slotIndex, targetPlayerIndex, hostCardId, targetCardId) {
    ...
    // 暂存卡牌到槽位（效果延迟至全员揭示后按顺序结算）
    fieldOwner.field[slotIndex].card = card
  ...
  revealAllCards() {
    ...
    const batch = EffectManager.buildRevealBatch(this.gameState)
    const messages = []
    const resolved = []

    for (const entry of batch) {
      const player = this.gameState.players[entry.playerIndex]
      const fieldOwner = this.gameState.players[entry.fieldOwnerIndex]
      console.log(`[GameEngine] 结算 ${player.name} 的 ${entry.card.name}（当前费用=${player.currentCost}）`)

      if (player.currentCost < 0) {
        EffectManager.voidPendingRevealEntry(entry, this.gameState, messages)
        continue
      }
    ...
    this.gameState.lastResolvedBatch = resolved.map(e => ({ ...e }))
    this.gameState.isResolvingRevealBatch = true
    for (const entry of resolved) {
      ...
      for (const effect of deferred) {
        const r = EffectManager.applyRevealEffect(effect, entry.card, player, this.gameState, undefined, resolved)
        messages.push(...r.messages)
      }
    }
    messages.push(...EffectManager.applyBatchHighestFreeDeploy(resolved, this.gameState))

    if (this.gameState.pendingBatchTacticDiscards?.length) {
      for (const pending of this.gameState.pendingBatchTacticDiscards) {
        const player = this.gameState.players.find(p => p.id === pending.playerId)
        if (!player) continue
        const slot = player.field[pending.slotIndex]
        if (slot?.card?.id === pending.card.id) slot.card = null
        if (!player.discard.includes(pending.card)) player.discard.push(pending.card)
        messages.push(`${pending.card.name} 揭示后进入弃牌区`)
      }
      this.gameState.pendingBatchTacticDiscards = []
    }
    this.gameState.isResolvingRevealBatch = false

    Object.keys(this.gameState.pendingReveals).forEach(id => {
      this.gameState.pendingReveals[id] = []
    })

    if (messages.length > 0) {
      this.gameState.message = (this.gameState.message || '') + ' | ' + messages.join(' | ')
      messages.forEach(msg => console.log(`[GameEngine] 揭示: ${msg}`))
    }
  ...
  resolveStagedPlay(card, player, slotIndex, playerIndex, fieldOwnerIndex) {
    const slot = player.field[slotIndex]
    if (!slot?.card) slot.card = card
    ...
    EffectManager.applyOnFieldDestroy(this)
    EffectManager.recalculateAllPowers(this.gameState)
    this.checkFieldFull(fieldOwnerIndex ?? playerIndex)
  }
    ...
    this.gameState.players.forEach((player, idx) => {
      ...
      if (playerPendingReveals.length > 0) {
        ...
        playerPendingReveals.forEach(reveal => {
          hiddenCost += reveal.playCost ?? reveal.card.cost
