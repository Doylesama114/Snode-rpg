...
articles = set(re.findall(r'<article class="skill(?: starting)?" id="(p-[^"]+)"', html))
...
print(f"\nSkills without 标识 ({len(no_mark)}): {no_mark[:12]}{'...' if len(no_mark) > 12 else ''}")
...
print("\n=== Spot checks ===")
for sid, expected_name, has_mark in checks:
    sk = next((s for s in data["skills"] if s["id"] == sid), None)
    if not sk:
        print(f"  {expected_name} ({sid}): MISSING")
        continue
