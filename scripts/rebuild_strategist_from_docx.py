#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 基础职业-谋士.docx 重建谋士 JSON + HTML + skill_effects。

新职业无历史 HTML 可增量 patch，按守望者脚本同款模板从零生成标准职业页
（起始特性 + 权谋/军团/先见/鸩毒/混乱/博物 六风格）。
"""
...
TIER_ORDER = ["一阶", "二阶", "三阶"]
STARTING_ORDER = ["交友术", "战术部署", "毒刃", "离间"]
FEATURE_NAMES = ["运筹帷幄", "博闻强识", "料敌机先"]
...
def candidate_names() -> set[str]:
    ...
    for tb in doc.tables:
        ...
        if text and text not in exclude and not text.startswith(("·", "1.", "2.")):
            names.add(text)
    return names


def collect_stubs() -> list[dict]:
    ...
    for idx, stub in enumerate(ordered_starting, 1):
        stub["id"] = f"{PREFIX}-starting-skill-{idx}"
    ...
    for idx, stub in enumerate(style_skills, 1):
        stub["id"] = f"{PREFIX}-skill-{idx}"

    return ordered_starting + style_skills
...
def block_to_skill(stub: dict, block: dict) -> dict:
    ...
    if stub["id"].startswith(f"{PREFIX}-starting-skill-"):
        skill["type"] = "starting"
    else:
...
def main() -> None:
    ...
    for stub in stubs:
        pick_style = "" if stub["id"].startswith(f"{PREFIX}-starting-skill-") else stub["style"]
