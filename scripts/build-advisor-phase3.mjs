...
import { buildAdvancementsCatalog } from './build-advisor-advancements.mjs';
...
function mergeAdvancements() {
  return buildAdvancementsCatalog({ write: false });
}
...
function main() {
  ...
  const advancementsDoc = mergeAdvancements();
  ...
  console.log(`  advancements: ${advancementsDoc.meta.count} scopes=${JSON.stringify(advancementsDoc.meta.scopeCounts || {})}`);
