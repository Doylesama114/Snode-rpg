...
3. 来源行是硬性要求：**无论回答长短，最后一行必须以「来源：」开头**；格式为「来源：条目名1 / 条目名2（出处：职业页·技能 / 规则手册·章节）」；只列本次实际引用的条目名，最多 5 个；未引用任何条目则写「来源：无（资料未收录）」；**不得用【参考】列表替代来源行**。
4. 回答首行标注资料档位：「资料档位：full」或「资料档位：partial」或「资料档位：basic」。full 可给出完整 build 建议；partial 需在建议后补一句「部分资料未收录，深度搭配请与 DM 确认」；basic 仅作通用创建陪跑。
5. 缺关键信息（职业/等级/技能名）时**禁止只反问**：必须先依据现有上下文给出 2–3 个候选方向或通用判断，再提出追问，并**照常输出档位行与来源行**。
6. 当用户输入疑似错别字或口语简称（检索不到完全匹配的条目）时，**先列出最可能的 2 个候选条目名**请用户确认，再按最可能的那个给出初步内容，并标注「（按 X 推测）」。
7. 被问「怎么加点/怎么配/怎么选」时，除规则说明外**必须给出 1–2 套具体分配示例**（如「力量 16 / 体质 14 / 其余 12」），并注明「示例，按队伍与风格调整」。
   无论哪种情况都禁止编造名称或数值。
...
9. build/规则/技能类回答末尾必须输出「【参考】」行：格式 【参考】名称1（职业·条目id）｜名称2（职业·条目id）。**只允许列出上下文中带「条目:」标记的条目**，id 必须从「条目:」后原样复制，不得编造、不得改写格式；引不出合法条目时宁可不写该条，也不要自造 id。涉及规则时可加「名称（规则）」，此类无需条目 id。聊天类问题不强制。
10. 检索上下文已包含问题涉及的内容（如同名技能/机制/特性条目）时，**禁止使用「当前资料未收录」句式**，必须基于上下文作答；「未收录」仅适用于上下文确实没有任何相关内容的情形。上下文已出现的名称/机制不得被说成"未写明"或"未收录"。`;
...
- 技能/效果罗列类回答（列完技能效果后）末尾必须补 1 行「优先选择建议」，说明按什么原则取舍（如输出/生存/控场）。
...
const INTENT_ADDONS = {
  ...
  point_buy_optimize: `
## 本问类型：购点优化
- 先确认职业关键属性并给出目标值；32 点购点内分配建议须具体到属性项。
- 说明种族加值在购点后叠加：最终值 = 购点值 + 种族加值（如购点智力15 + 地精+2 = 17），不要只写购点值。
- 若上下文缺少种族/职业信息，先说明缺失项再给通用原则。`,
  chargen_recommend: `
## 本问类型：车卡推荐（chargen_recommend）
- 从【职业定位】中推荐 3-4 个不同职业，逐个说明定位、操作感来源、上手难度、生存能力；不要只推荐一个职业。
- 每个推荐职业附 2-3 个推荐种族（从【种族】表选），说明契合点：综合属性加值与种族特性（如法术命中、回复疲劳、位移、豁免等），不要只看属性加值。
- **必须**给每个推荐职业写出属性目标的最终值：购点值 + 种族加值 = 最终值（如游荡者购点敏捷 15 + 木精灵 +2 = 17），不得只写购点值。
- 可推荐 1-2 个背景（从【背景】表选），推荐后必须加一句：「如果你有想兼职的子职业或者想玩的特定流派，那么可以不必参考此次个性背景的推荐，自行选择最适合你的个性背景」。
- 职业、种族、背景名称必须来自本上下文列表，不得编造上下文未出现的名称。
- 推荐是建议，职业与种族/背景不绑定；可提醒玩家按自己的喜好调整。`,
  entity_qa: `
## 本问类型：实体百科
...
- 仅曙光年代可玩奇械师；勿引用塑能箭等法师专属技能；智力为关键属性（FP 关键属性以创建页为准）。`,
  sorcerer_skills: `
## 本问类型：术士技能
- 引用技能名、战斗风格（潜能等）、阶位须与 L2 术士上下文一致；勿套用法师八学派名。
- 混沌法术是随机法术：先说明掷 D100 决定效应，再按点数区间完整列出对应效果（001 / 002-005 / 006-010 / 011-020 / 021-030 … 直到 100），不要只举一两个区间。
- 混乱箭与混沌法术联动：混乱箭需先通过混沌法术掷出 5 以下或 95 以上的效应才可释放。
- 起始特性（魔法飞弹/混沌法术/天赐神通）与 1 阶技能（混乱箭/魔爆术等）区分；关键属性为幸运。`,
  warlock_skills: `
## 本问类型：魔契师技能
...
- 治愈和弦等回复类技能回答须注明关键词含「媒介」与「节奏」（以 L2 上下文为准）。
...
- 回答某一风格后，末尾用一行说明该风格与其他武僧风格（踏风/织雾/无尘/锋岚/酒仙/凰火）的差异（以上下文为准）。
...
- 回答某一元素风格后，末尾用一行区分其他元素风格（火焰/水源/大地/巫术，以上下文为准）。
...
- 野兽形态升级须完整转述上下文内容：水栖形态提供水下呼吸、生命值上限临时变为 5+角色等级、水下移动速度 8 米；5 级可变形为海豹/海豚/小型鲨鱼等更具攻击性的水栖生物（以上下文为准，勿遗漏数值）。
...
- 区分相似状态（如沉默 vs 耳鸣）；勿编造未收录技能或效果。`,
  worldview_lore: `
## 本问类型：世界观 / 设定（worldview_lore）
- 仅依据【检索上下文】中「## 世界观」段落作答；这是官方设定摘要，不是玩家经验贴。
- 可按章节组织（年代、雷恩国情、神祇、节日、周边政权、组织等）；上下文未出现的地名/神名/剧情**不得编造**。
- 地图图本身无文字细节；若仅有地图章节标题而无正文，说明设定以帮助页地图为准、文字资料不足。
- 战役私密主线、未公开剧透应交由 DM；结尾可一句「以帮助页世界观与 DM 为准」。`,
  skill_aggregate: `
## 本问类型：跨职业同名技能聚合
...
export function buildSystemPrompt(options = {}) {
  ...
  let addon = mode === 'wizard'
    ? INTENT_ADDONS.wizard
    : (INTENT_ADDONS[profile] || INTENT_ADDONS[intent] || '');
  // 职业专属 addon：非技能意图（升级/进阶等）也合并类内细节规则，避免遗漏升级/形态细节
  const classProfileName = className ? getClassProfile(className).promptProfile : null;
  if (classProfileName && classProfileName !== profile && INTENT_ADDONS[classProfileName]) {
    addon = `${addon}\n${INTENT_ADDONS[classProfileName]}`;
  }

  const clarifyNote = options.clarify?.needs?.length
    ? `\n## 缺少关键信息（交互追问）\n- 用户尚未提供：${options.clarify.needs.map((n) => n.prompt).join('、')}。\n- 回答先用 1 行说明需要确认这些信息，再给通用方向；不要展开具体职业/等级专属建议。`
    : '';
...
${STYLE_RULES}
${addon}${clarifyNote}${goalOverrideNote}
${tierAudit ? `\n## 档位检查（勿向用户复述标题）\n${tierAudit}\n` : ''}
