...
function buildIndex({ sourceFile, sourceKey, className, outFile }) {
...
function patchRulesSummary(mageCount, universalCount) {
  if (!fs.existsSync(RULES_SUMMARY)) return;
  const summary = loadJson(RULES_SUMMARY);
  const bullet = `L2 技能索引：法师 ${mageCount} + 通用 ${universalCount}（与 skill_effects 零名差）`;
...
function main() {
  ...
  patchRulesSummary(mage.meta.count, universal.meta.count);
