...
def main() -> int:
    ...
    if not adv.exists():
        ...
        if '召唤师专属进阶途径等待' not in t:
            errors.append('进阶页缺少等待更新提示')
        if len(re.findall(r'class="adv-card"', t)) != 10:
            errors.append('进阶页通用卡片 != 10')
    ...
    ok_notes.append('进阶占位页（通用 10 条 + 等待更新提示）/ 首页 + 搜索索引 + 清单数据 全部就位')
